...
ManualBarrier = {}
ManualBarrier.version = "1.9.0.2"
ManualBarrier.date = "12.12.2019"
	...
	if self.randomActive then
		...
		if self.randomRun == true then
			...
			if self.count > 0 then
				if self.randomActiveText ~= nil and self.randomActiveText ~= "" then
...
function ManualBarrier:saveToXMLFile(xmlFile, key, usedModNames)
	...
	setXMLString(xmlFile, key.."#randomActiveText", self.randomActiveText)
end
function ManualBarrier:getFieldOwnership()
	if self.farmLandRestricted then
		...
		if ownerFarmId == nil or farmId == 0 or farmId == AccessHandler.EVERYONE or g_currentMission.accessHandler:canFarmAccessOtherId(farmId, self.triggerObjectFarmId) then
			return true
		end
	else
		return true
...
function ManualBarrier:getFarmId()
	local x,_,z = getWorldTranslation(self.nodeId)
	local ownerFarmId = g_farmlandManager:getFarmlandIdAtWorldPosition(x, z)
	local farmId = 0
	if ownerFarmId ~= nil then
	...
	if g_currentMission ~= nil then
			...
			if vehicle ~= nil then
				...
				if self:getFieldOwnership() then
					if onEnter and self.isEnabled then
						self.count = self.count + 1
						if vehicle.getIsControlled ~= nil and vehicle:getIsControlled() ~= nil and self.manualOpen and pName ~= nil and pName == vehicle:getControllerName() then
							self.playerInTrigger = true
						end
					elseif onLeave then
						self.count = self.count - 1
